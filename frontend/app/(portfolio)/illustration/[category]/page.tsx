...
export default function IllustrationCategoryPage({ params }: PageProps) {
  ...
  const { data, mutate, isLoading } = useSWR(
    `/api/artworks?category=illustration&subcategory=${category}`,
    fetcher
  );
      ...
      {artworks.length > 0 ? (
        ...
        <Empty>
          <EmptyContent>
            ...
            <EmptyTitle>등록된 일러스트가 없습니다</EmptyTitle>
            <EmptyDescription>새 일러스트를 추가하여 갤러리를 채워보세요.</EmptyDescription>
          </EmptyContent>
        </Empty>
      )}
    </div>
